...
class MainActivityViewModel @Inject constructor(
    ...
    fun translateText(userPickedLanguage: String) {
        viewModelScope.launch {
            translateTextUseCase(
                TranslateRequest(
                    text = "This is a test translation.",
                    targetLanguages = listOf(userPickedLanguage)
                )
            ).execute(onSuccess = { response ->
                _mainScreenState.update {
                    it.copy(
                        isLoading = false,
                        //translatedTranscript = it.translatedTranscript.plus(response.translations)
                    )
                }
